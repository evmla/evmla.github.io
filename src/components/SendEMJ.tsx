...
const Send = () => {
  ...
  const [sender, setSender] = useState<Emoji | undefined>();
  const [recipient, setRecipient] = useState<Emoji | undefined>();
  ...
  useEffect(() => {
    const [owner, emoji, name] = senderRaw?.[0]?.value?.[0] ?? [
      null,
      null,
      null,
    ];
    if (owner === ethers.constants.AddressZero) {
      ...
      setSenderLoading(false);
      setSender({ owner, emoji, name });
    }
  }, [senderRaw]);

  useEffect(() => {
    const [owner, emoji, name] = recipientRaw?.[0]?.value?.[0] ?? [
      null,
      null,
      null,
    ];
        ...
        <Grid justify="center" align="center">
          <Grid.Col span={3} style={{ textAlign: "center" }}>
            {senderLoading ? (
              ...
              <Text size={60}>{(sender && sender.emoji) || "❓"}</Text>
            )}
          </Grid.Col>
          <Grid.Col span={6}>
